...
public class HashRingBuffer <T> {
	 ...
	 * 				at a given time
	 * ringBuffer:	HashMap to store elements
	 */
	...
	public HashRingBuffer() {
		this.init();
		ringBuffer = new HashMap<Integer, T>(DEFAULT_SIZE);
		this.fill();
	}
	...
	public HashRingBuffer(int size, T...ts) {
		if (size <= 0) {
			throw new IllegalArgumentException("New size: " + size);
		}
		this.reset(size);
	...
	public void reset(int psize) {
		if (psize <= 0) {
			throw new IllegalArgumentException("New size: " + psize);
		}
	...
	public boolean append(T elem) {
		...
		if (this.currentSize != this.size) {
			...
			// head and tail are the same, hence increment tail
